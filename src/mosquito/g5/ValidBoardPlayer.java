...
public class ValidBoardPlayer extends mosquito.sim.Player {
	...
	public boolean[][] createValidBoard( int[][] board ) {
		validBoard = new boolean[100][100];
		for ( int i = 0; i < 100; i++ ) {
			for ( int j = 0; j < 100; j++ ) {
				validBoard[i][j] = true;
			}	
		...
		for ( Line2D wall : walls ) {
			...
			// add points in line
			for(int x = (int)(p1.getX()); x < p2.getX() + .5; x++){
				int y  = (int)( (x-p1.getX())*slope + p1.getY() +.5);
				//add (x,y) to the valid board
				// The Cross of Disallowment
				for ( int n = -10; n <=10; n++ ) {
					if ( y+n >= 0 && y+n < 100 ) {
						validBoard[x][y+n] = false;
					}
					if ( x+n >= 0 && x+n < 100 ) {
						validBoard[x+n][y] = false;
					}
				}
	...
	public void printBoard(boolean[][] v)
	{
		for (int i = 0;i<100;i++)
		{
			for(int j = 0;j<100;j++)
	...
	public Set<Light> getLights(int[][] board) {
		...
		this.validBoard  = createValidBoard(board);
		sectors = getSectors();
	...
	public HashMap<Integer, HashSet<Tuple<Integer,Integer>>> getSectors()
	{
		...
		Iterator<Tuple<Integer,Integer>> iter = s.iterator();
		while(iter.hasNext())
		{
		...
		if ( currentMove % 2 == 0 ) {
			...
			for(int i = 0; i < numLights; i++)
			{
				...
				if ( allMosquitoesCaptured(board) ) {
					nextStop = new Tuple<Integer,Integer>(50,51);
				}			
				...
				if(lightArr[i].getX() == o.get(0).x && lightArr[i].getY() == o.get(0).y)
					o.remove(0);
			}
		}
